const {
  app,
  webFrame,
  commandLine,
  BrowserWindow,
  globalShortcut,
} = require('electron');

function createWindow () {
  // Create the browser window.
  const win = new BrowserWindow({
    resizable: false,
    frame: false,
    fullscrenn: true,
    kiosk: true,
    closable: false,
    maximizable: false,
    minimizable: false,
    fullScreenable: false,
    movable: false,
    webPreferences: {
      nodeIntegration: true
    }
  })

  // and load the index.html of the app.
  win.loadFile('build/index.html')
  // Open the DevTools.
  /*
  if (process.env.NODE_ENV !== 'production') {
    win.webContents.openDevTools()
  }
  */

  webFrame.setZoomFactor(1);
  webFrame.setZoomLevelLimits(1, 1);
  webFrame.setVisualZoomLevelLimits(1, 1);
  webFrame.setLayoutZoomLevelLimits(1, 1);
  commandLine.appendSwitch('disable-pinch');

  globalShortcut.register('Super', () => false);
  globalShortcut.register('alt+tab', () => false);
  globalShortcut.register('alt+f5', () => false);
  globalShortcut.register('CommandOrControl+tab', () => false);
  globalShortcut.register('f5', () => false);
  globalShortcut.register('CommandOrControl+R', () => false);
  globalShortcut.register('CommandOrControl+alt+f', () => false);
  globalShortcut.register('CommandOrControl+f', () => false);
  globalShortcut.register('CommandOrControl+-', () => false);

  win.setMenu(null);

  win.webContents.on('zoom-changed', () => {
    webFrame.setZoomFactor(1);
  });

  win.webContents.on('leave-full-screen', e => {
    if (e && e.preventDefault) e.preventDefault();
    return false;
  });

  win.on('blur', () => {
    win.show();
  });

  win.on('close', e => {
    e.preventDefault();
    return false;
  });
}

// This method will be called when Electron has finished
// initialization and is ready to create browser windows.
// Some APIs can only be used after this event occurs.
app.whenReady().then(createWindow)

// Quit when all windows are closed.
app.on('window-all-closed', () => {
  // On macOS it is common for applications and their menu bar
  // to stay active until the user quits explicitly with Cmd + Q
  if (process.platform !== 'darwin') {
    app.quit()
  }
})

app.on('activate', () => {
  // On macOS it's common to re-create a window in the app when the
  // dock icon is clicked and there are no other windows open.
  if (BrowserWindow.getAllWindows().length === 0) {
    createWindow()
  }
})

// In this file you can include the rest of your app's specific main process
// code. You can also put them in separate files and require them here.
